...
import { removeFromBasket, selctBasketTotal, selectBasketItems } from '../slices/basketSlice';
...
useEffect(()=>{
    const groupdedItems=items.reduce((res,item)=>{
        (res[item.id]= res[item.id]||[]).push(item)
        return res;
    },{})

    setGroupedItemsInBasket(groupdedItems);
},[items])
    ...
    <View className='flex-1 bg-white'>
        <View className='flex-1 bg-gray-100'>
         ...
         <View className='flex-row items-center
          space-x-4 px-4 py-3 bg-white my-5'>
            <Image
            source={{
        ...
        <View className='p-5 pb-12 bg-white mt-5 space-y-4'>
            ...
            <TouchableOpacity
             ...
             onPress={()=>navigation.navigate('PreapperingOrderScreen')}
             >
